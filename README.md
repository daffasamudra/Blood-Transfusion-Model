...
* **Sumber:** [UCI Machine Learning Repository (Blood Transfusion Service Center)](https://archive.ics.uci.edu/ml/datasets/Blood+Transfusion+Service+Center)
...
| Fitur | Deskripsi |
| :--- | :--- |
| **Recency (R)** | Bulan sejak donasi terakhir. |
| **Frequency (F)** | Total jumlah donasi. |
| **Monetary (M)** | Total volume darah (c.c.). |
| **Time (T)** | Bulan sejak donasi pertama kali (lama menjadi anggota). |
| **Target** | `1` = Mendonor kembali, `0` = Tidak mendonor. |
...
1. **Data Cleaning:** Memeriksa missing values (Dataset bersih/tidak ada missing value).
2. **Feature Selection:** Menghapus fitur *Monetary* karena memiliki korelasi sempurna (1.0) dengan *Frequency*, sehingga informasi menjadi redundan.
3. **Handling Imbalance:** Menggunakan teknik **SMOTE** (*Synthetic Minority Over-sampling Technique*) pada data training untuk menyeimbangkan kelas minoritas (Donor) agar model tidak bias.
4. **Scaling:** Menggunakan `StandardScaler` untuk menormalisasi fitur *Recency*, *Frequency*, dan *Time*. Langkah ini krusial untuk algoritma KNN dan Neural Network.
5. **Splitting:** Data dibagi menjadi **80% Training** dan **20% Testing**.
...
* Menggunakan algoritma *K-Nearest Neighbors* sebagai pembanding dasar.
* Model ini mengklasifikasikan berdasarkan mayoritas kelas dari tetangga terdekat.

**Model 2 – Advanced ML (Random Forest)**
* Menggunakan *Random Forest Classifier* (*Ensemble Method*).
* Model ini dipilih karena kemampuannya menangani outlier dan memberikan informasi *feature importance*.

**Model 3 – Deep Learning (MLP)**
* Menggunakan *Multilayer Perceptron* (Neural Network) dengan framework TensorFlow/Keras.
* Arsitektur terdiri dari Input Layer, Hidden Layers dengan aktivasi ReLU, Dropout untuk mencegah overfitting, dan Output Layer dengan aktivasi Sigmoid.
...
| Model | Accuracy Score | Analisis Singkat |
| :--- | :---: | :--- |
| **KNN (Baseline)** | 0.72 | Masih menghasilkan banyak False Negative. |
| **Random Forest** | 0.73 | Stabil, menemukan bahwa fitur 'Time' sangat berpengaruh. |
| **Deep Learning** | **0.77** | **Model Terbaik.** Mampu menangkap pola data lebih baik dibanding metode tradisional. |
...
* [ ] **Menambah Data:** Mengumpulkan lebih banyak data historis untuk meningkatkan performa model Deep Learning.
* [ ] **Hyperparameter Tuning:** Melakukan GridSearch atau Bayesian Optimization untuk mencari parameter terbaik bagi Random Forest dan MLP.
* [ ] **Deployment:** Membuat aplikasi web sederhana menggunakan Streamlit agar model dapat digunakan oleh staf medis secara langsung.
...
1. **Clone Repository:**
   ```bash
   git clone [https://github.com/daffasamudra/Blood-Transfusion-Model.git](https://github.com/daffasamudra/Blood-Transfusion-Model.git)
   ```
